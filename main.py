...
def __help(message):
    ...
    text = f'✔️ Помощь и инструкция по боту:✔️ \n\n' \
           ...
           f'⭐ Риски в "Финансовой жизни" - /sovet\n\n' \
...
def return_sovet(message):
    user = message.from_user
    bot.send_message(user.id, f'Совет дня: {function_sovet_last()}')
...
def search_osn(message):
    ...
    if len(command_args) < 2:
        ...
        if slovar_find(slovar=slovar_unsorted, a=command_args[1]):
            bot.send_message(user.id,
                             f'{command_args[1]} - это {slovar_find(slovar=slovar_unsorted, a=command_args[1])}')
